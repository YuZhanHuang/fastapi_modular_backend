...
- 在 infra/containers/ 中被組裝進 core/services
...
- infra/containers/ 組裝到 core/services
...
### 2.4 infra/containers/

#### 角色

Composition Root / IoC 容器（dependency-injector）。

負責把「抽象」與「實作」對應起來，以 DeclarativeContainer 集中管理 Provider。

#### 內容示例

定義：

- `cart_repository = providers.Factory(CartRepositoryImpl, session=...)`
...
- FastAPI / Pydantic
- infra.containers（取得 service 實例）
...
- 依賴 infra.containers（取得 service 實例）
- 使用 FastAPI Depends
...
- 依賴 infra.containers、core.services
...
- **integration/**：測 infra 實作與 DB/外部服務整合（標記 `@pytest.mark.integration`）
- **bdd/**：從使用者行為角度驗證完整流程（`tests/features/` + `tests/step_defs/`）

### 規範

- 測試可以跨層引用，只要清楚測試目標
- 優先保證 core 的穩定與正確，降低 refactor 成本
- 需真實 Postgres/Redis 的測試才標記 `integration`；其餘保持輕量（stub、SQLite、TestClient）

### 執行方式

測試統一走 Docker，詳見 [測試流程說明](測試流程說明.md)。
...
# 指定檔案
./scripts/test.sh tests/test_exception_handlers.py -v
...
### 11.1 Docker Compose 檔案

| 檔案 | 職責 |
|------|------|
| `docker-compose.yml` | base：db、redis、app、worker、beat、flower；`build.target: base`；db/redis 含 healthcheck |
| `docker-compose.dev.yml` | 開發覆寫：四個 Python 服務改 `dev` target，掛載 `src/`、`tests/` |
| `docker-compose.test.yml` | 測試：`test`（輕量 standalone）、`test-integration`（`integration` profile） |

Dockerfile 提供 `base`（部署）與 `dev`（含 pytest 等 dev 依賴）兩個 stage。

### 11.2 開發環境啟動

```bash
# 推薦：腳本已合併 dev compose
./scripts/dev-up.sh
./scripts/dev-down.sh
...
僅使用 `docker compose up -d`（不含 dev overlay）會啟動 **base image**，無 dev 依賴與 source volume，不適合日常開發。

### 11.3 容器內啟動流程

1. `entrypoint.sh` 執行
2. 等待資料庫/Redis 就緒
3. 執行 Alembic migrations（`RUN_MIGRATIONS=1` 時，通常由 app 服務執行）
4. 依 `SERVICE_ROLE` 啟動服務：
...
測試服務不走 `entrypoint.sh`，改由 `scripts/run-tests.sh` 直接執行 pytest（integration 時會先跑 migration）。
...
| **Composition Root** | `application/app.py`<br>`infra/containers/` | 組裝各層的入口點 |
...
6. ✅ **Composition Root** - `infra/containers/`、`application/app.py` 已實現
7. ✅ **依賴方向** - 已正確實現
8. ✅ **業務邏輯獨立性** - 已實現
...
| Composition Root | ✅ 100% | `application/`、`infra/containers/` 完整實現 |
